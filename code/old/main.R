...
# Convert tariffs to long data.frame 
...
baseline_filename <- "output/baseline_results.rds"
if (file.exists(baseline_filename)) {
  baseline_results <- readRDS(baseline_filename)
} else {
baseline_results <- calibrate_multisector_baseline(T_matrix = T_matrix, 
      ...
      saveRDS(baseline_results, "output/baseline_results_imobile.rds")
}
...
# Example: Increase US tariffs by 10 percentage points across all sectors
# Create counterfactual tariff matrix
counterfactual_tariffs <- cleaned_data$tariffs
us_index <- which(countries == "USA")
counterfactual_tariffs[us_index, ] <- counterfactual_tariffs[us_index, ] + 0.10

# Run counterfactual simulation with increased US tariffs
counterfactual_results <- run_counterfactual_scenario(
  baseline_results = baseline_results,
  T_matrix = T_matrix,
  cleaned_data = cleaned_data,
  counterfactual_tariffs = counterfactual_tariffs,
  theta = theta,
  labor_mobility = TRUE
)
