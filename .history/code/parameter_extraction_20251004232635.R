...
estimate_cdk_technology_params <- function(regression_data, theta = 6.53) {
    ...
    # Handle missing values using matrix completion
    if (any(is.na(T_composite_matrix))) {
        # Simple mean imputation for missing technology parameters
        for (i in 1:n_countries) {
            for (k in 1:n_sectors) {
                if (is.na(T_composite_matrix[i, k])) {
                    sector_mean <- mean(T_composite_matrix[, k], na.rm = TRUE)
                    country_mean <- mean(T_composite_matrix[i, ], na.rm = TRUE)
                    T_composite_matrix[i, k] <- ifelse(is.na(sector_mean), 
                                                      ifelse(is.na(country_mean), 1, country_mean), 
                                                      sector_mean)
                }
            }
        }
