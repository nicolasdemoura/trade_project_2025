...
print(alpha_xtable, file = "tables/alpha_table.tex", include.rownames = TRUE, caption.placement = "top", sanitize.text.function = identity, # allows LaTeX symbols
    align = c("l", "l", rep("c", ncol(model_parameters$alpha) - 1)),
    add.to.row = list(
...
print(beta_xtable, file = "tables/beta_table.tex", include.rownames = TRUE, caption.placement = "top", sanitize.text.function = identity, # allows LaTeX symbols
    align = c("l", "l", rep("c", ncol(model_parameters$beta) - 1)),
    add.to.row = list(
...
for(j in 1:dim(model_parameters$gamma)[1]) {
    ...
    print(gamma_xtable, 
          ...
          align = c("l", "l", rep("c", ncol(gamma_matrix) - 1)),
          add.to.row = list(
...
print(T_xtable, file = "tables/technology_table.tex", include.rownames = TRUE, caption.placement = "top", sanitize.text.function = identity, # allows LaTeX symbols
    align = c("l", "l", rep("c", ncol(model_parameters$technology) - 1)),
    add.to.row = list(
...
for(i in 1:dim(processed_database$tariffs)[1]) {
    ...
    tariff_xtable <- xtable(tariff_vector, 
                            ...
                            label = paste0("tab:tariffs_", country))
    print(tariff_xtable, 
          file = paste0("tables/tariff_table_", country, ".tex"),
          include.rownames = TRUE, 
          caption.placement = "top", 
          sanitize.text.function = identity, # allows LaTeX symbols
          align = c("l", "l", rep("c", ncol(tariff_vector) - 1)),
          add.to.row = list(
            pos = list(nrow(tariff_vector)),
            command = paste0("\\hline \\multicolumn{",
                    ncol(tariff_vector) + 1,
