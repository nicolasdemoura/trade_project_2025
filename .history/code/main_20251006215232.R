...
print(alpha_xtable, file = "tables/alpha_table.tex", include.rownames = TRUE, caption.placement = "top", sanitize.text.function = identity, # allows LaTeX symbols
    align = c("l", rep("c", ncol(model_parameters$alpha))),
    add.to.row = list(
...
T_xtable <- xtable(model_parameters$technology, caption = "Technology Parameters", label = "tab:technology")
print(T_xtable, file = "tables/technology_table.tex", include.rownames = TRUE, caption.placement = "top", sanitize.text.function = identity, # allows LaTeX symbols
    align = c("l", rep("c", ncol(model_parameters$technology))),
    add.to.row = list(
      pos = list(nrow(model_parameters$technology)),
      command = paste0("\\hline \\multicolumn{",
              ncol(model_parameters$technology) + 1,
              "}{l}{\\footnotesize This table presents the technology parameters for each country-sector pair.} \\\\")
    )
)
...
for(i in 1:dim(processed_database$tariffs)[1]) {
    ...
    tariff_matrix <- matrix(tariff_vector, nrow = 1)
    colnames(tariff_matrix) <- colnames(processed_database$tariffs)
    rownames(tariff_matrix) <- country
    
    tariff_xtable <- xtable(tariff_matrix, 
                           caption = paste0("Tariff Rates - ", country), 
                           label = paste0("tab:tariff_", country))
    print(tariff_xtable, 
          file = paste0("tables/tariff_table_", country, ".tex"), 
          include.rownames = TRUE, 
          caption.placement = "top", 
          sanitize.text.function = identity, # allows LaTeX symbols
          align = c("l", rep("c", ncol(tariff_matrix))),
          add.to.row = list(
            pos = list(nrow(tariff_matrix)),
            command = paste0("\\hline \\multicolumn{",
                    ncol(tariff_matrix) + 1,
                    "}{l}{\\footnotesize This table presents the tariff rates for ", 
                    country, " across all sectors.} \\\\")
          )
    )
}
