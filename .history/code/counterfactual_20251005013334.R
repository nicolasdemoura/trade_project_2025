...
solve_counterfactual_equilibrium <- function(database, parameters, fixed_iceberg_costs, labor_mobility = TRUE) {
    ...
    # Initialize wages based on labor mobility
    if (labor_mobility) {
    ...
    counterfactual_objective <- function(params) {
        ...
        if (labor_mobility) {
            ...
            for (i in 1:n_countries) {
                ...
                violations <- violations + (lhs - rhs)^2
            }
        } else {
        ...
        for (n in 1:n_countries) {
            for (k in 1:n_sectors) {
                ...
                model_price <- price_index_sum^(-1/theta)
                violations <- violations + (prices[n, k] - model_price)^2
            }
        }
