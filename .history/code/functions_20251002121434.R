...
#' @return New trade cost matrix with tariffs (domestic costs remain 1)
apply_tariffs <- function(tau_baseline, tariff_matrix) {
  new_tau <- tau_baseline * (1 + tariff_matrix)
  
  # Ensure domestic trade costs remain exactly 1
  if (length(dim(new_tau)) == 3) {
    # 3D array case (countries x countries x sectors)
    n_countries <- dim(new_tau)[1]
    n_sectors <- dim(new_tau)[3]
    for (s in 1:n_sectors) {
      for (i in 1:n_countries) {
        new_tau[i, i, s] <- 1.0
      }
    }
  } else if (length(dim(new_tau)) == 2) {
    # 2D matrix case (countries x countries)
    n_countries <- dim(new_tau)[1]
    for (i in 1:n_countries) {
      new_tau[i, i] <- 1.0
    }
...
#' Wrapper for multi-sector equilibrium solver (compatibility with counterfactual functions)
#' @param theta_vec Vector of trade elasticities by sector (uses first element)
#' @param T_matrix Technology matrix
#' @param tau_array Trade cost array
#' @param L_matrix Labor matrix
#' @param beta_vec Expenditure shares (converted to matrices if needed)
#' @param ... Additional arguments passed through
#' @return Equilibrium results compatible with existing code
solve_equilibrium_cdk_wrapper <- function(theta_vec, T_matrix, tau_array, L_matrix, beta_vec, ...) {
  
  # Use simplified approach for compatibility
  # This calls the original CDK solver for now
  return(solve_equilibrium_cdk(theta_vec, T_matrix, tau_array, L_matrix, beta_vec, ...))
}
