...
compute_counterfactual_equilibrium_cdk <- function(baseline_results, tariff_array, 
  ...
  if (!is.null(baseline_results$beta_matrix)) {
    ...
    # Re-optimize equilibrium with new tariffs using the same approach as baseline
    counterfactual_equilibrium <- calibrate_equilibrium_from_trade_shares(
      ...
      observed_trade_shares = baseline_results$equilibrium$trade_shares,  # Use baseline structure as target
      baseline_tariffs = tariff_array,  # New tariff structure
