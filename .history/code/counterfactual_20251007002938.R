...
export_welfare_tables <- function(welfare_results, baseline_equilibrium, database, output_dir = "results/welfare_tables") {
    ...
    for (scenario_name in names(welfare_results)) {
        ...
        welfare_table <- data.frame(
            ...
            Income_Change_Pct = round((welfare_effects$income_change / welfare_effects$baseline_real_income) * 100, 2),
            Tariff_Revenue_Change_Pct = round((welfare_effects$tariff_revenue_change / welfare_effects$baseline_tariff_revenue) * 100, 2),
