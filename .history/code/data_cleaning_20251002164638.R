...
clean_socioeconomic_data_cdk <- function(socioeco_file_path, exchange_rates_file_path, target_year = 2011, country_groups = NULL, sector_groups = NULL) {
    ...
    socioeco_data <- socioeco_data %>%
        ...
        select(Country, Variable, Description, Code, paste0("_", target_year))

        

    # Load exchange rates data
    exchange_rates <- readxl::read_excel(exchange_rates_file_path, sheet = 2)
    exchange_rates <- as.data.frame(exchange_rates)
    exchange_rates <- exchange_rates %>%
        mutate(across(starts_with("_"), as.numeric))

    # Convert monetary values to USD using exchange rates
    if (paste0("_", target_year) %in% names(exchange_rates)) {
      rate_col <- paste0("_", target_year)
      rates <- exchange_rates %>%
        select(Country, all_of(rate_col)) %>%
        rename(exchange_rate = all_of(rate_col))
      
      socioeco_data <- socioeco_data %>%
        left_join(rates, by = "Country") %>%
        mutate(across(starts_with("_"), ~ ifelse(Variable %in% c("LAB", "COMP", "GO", "VA"), .x / exchange_rate, .x))) %>%
        select(-exchange_rate)
    } else {
      warning(paste("Exchange rate for year", target_year, "not found. Monetary values not converted."))
    }
    ...
    gdp_data <- socioeco_data %>%
      ...
      select(Country, Code, paste0("_", target_year))
