...
clean_socioeconomic_data_cdk <- function(socioeco_file_path, target_year = 2011, country_groups = NULL) {
  ...
  employment_data$value <- as.numeric(employment_data$value)
  ...
  beta_data <- lab_data %>%
    ...
    mutate(beta_ik = ifelse(va > 0, value / va, 0)) %>%
    select(Country, Code, beta_ik)
  
  # Create beta matrix (countries x sectors)
  beta_matrix <- beta_data %>%
    filter(Country %in% countries, Code %in% sectors) %>%
    pivot_wider(names_from = Code, values_from = beta_ik, values_fill = 0) %>%
    column_to_rownames(var = "Country") %>%
    as.matrix()
