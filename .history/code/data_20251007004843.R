...
create_latex_table <- function(data, filename, caption, label, output_dir = "tables", labor_mobility = TRUE) {
  ...
  full_path <- file.path(output_dir, paste0(filename, ifelse(ifelse(labor_mobility == NULL, ".tex", ifelse(labor_mobility, "_mobile.tex", "_immobile.tex")))))
...
export_parameter_tables <- function(processed_database, model_parameters, output_dir = "tables", labor_mobility = TRUE) {
