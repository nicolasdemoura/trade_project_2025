...
moments_objective_function <- function(params, observed_trade_shares, technology_matrix, tariffs, 
    ...
    for (n in 1:n_countries) {
        for (k in 1:n_sectors) {
            ...
            if (price_index_sum > 0) {
                ...
                price_consistency_violations <- price_consistency_violations + (prices[n, k] - model_price)^2
            }
        }
    }
    ...
    objective_value <- trade_share_objective + trade_balance_violations * 1e-9 + price_consistency_violations * 1e-6
