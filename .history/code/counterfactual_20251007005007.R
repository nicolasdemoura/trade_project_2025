...
solve_counterfactual_equilibrium <- function(database, parameters, fixed_iceberg_costs, labor_mobility = TRUE) {
    ...
    counterfactual_objective <- function(params) {
        ...
        # Calculate sector_shares
        sector_shares <- matrix(0, n_countries, n_sectors)
        for (n in 1:n_countries) {
            for (k in 1:n_sectors) {
                sector_shares[n, k] <- alpha_matrix[n, k] * total_income[n]
            }
        }
...
export_welfare_tables <- function(welfare_results, baseline_equilibrium, database, output_dir = "results/welfare_tables", labor_mobility = TRUE) {
    ...
    # Create one table per scenario with countries as rows
    for (scenario_name in names(welfare_results)) {
        welfare_effects <- welfare_results[[scenario_name]]$welfare_analysis$welfare_effects
        
        # Calculate baseline and counterfactual tariffs that US charges each country
        baseline_tariffs <- database$tariffs
        counterfactual_tariffs <- welfare_results[[scenario_name]]$scenario_tariffs
        
        # Get trade shares for weighting
        baseline_trade_shares <- baseline_equilibrium$model_solution$trade_shares
        counterfactual_trade_shares <- welfare_results[[scenario_name]]$counterfactual_outcomes$trade_shares
        
        # Calculate sector expenditure shares for weighting
        baseline_sector_shares <- baseline_equilibrium$model_solution$sector_shares
        counterfactual_sector_shares <- welfare_results[[scenario_name]]$counterfactual_outcomes$expenditures
        
        baseline_sector_shares <- baseline_sector_shares[us_index, ] / sum(baseline_sector_shares[us_index, ])
        counterfactual_sector_shares <- counterfactual_sector_shares[us_index, ] / sum(counterfactual_sector_shares[us_index, ])
        
        # Calculate average tariffs for each country
        baseline_avg_tariffs <- numeric(length(countries))
        counterfactual_avg_tariffs <- numeric(length(countries))
        
        for (i in 1:length(countries)) {
            if (i != us_index) {
                # Baseline average tariff
                baseline_avg_tariffs[i] <- calculate_trade_weighted_tariffs(
                    baseline_tariffs, baseline_trade_shares, baseline_sector_shares, us_index, i, sectors
                )
                
                
                # Counterfactual average tariff  
                counterfactual_avg_tariffs[i] <- calculate_trade_weighted_tariffs(
                    counterfactual_tariffs, counterfactual_trade_shares, counterfactual_sector_shares, us_index, i, sectors
                )
            }
        }
        ...
        welfare_table <- data.frame(
            Baseline_Tariff = round(baseline_avg_tariffs * 100, 2),
            Counterfactual_Tariff = round(counterfactual_avg_tariffs * 100, 2),
            Welfare_Change_Pct = round(welfare_effects$welfare_change_pct, 2),
            Income_Change_Pct = round((welfare_effects$income_change / baseline_equilibrium$outcome_variables$real_income) * 100, 2),
            Tariff_Revenue_Change_Pct = round((welfare_effects$tariff_revenue_change / baseline_equilibrium$outcome_variables$real_tariff_revenue) * 100, 2),
            row.names = welfare_effects$country,
            stringsAsFactors = FALSE
        )
        colnames(welfare_table) <- c("Baseline Tariff (p.p.)", "Counterfactual Tariff (p.p.)", "Welfare Change (\\%)", "Income Change (\\%)", "Tariff Revenue Change (\\%)")
        # Create better scenario names for display
        good_name <- switch(scenario_name,
                   "tariff_rate24" = "2024 Tariffs",
                   "tariff_rate25_YTD" = "2025 YTD Tariffs", 
                   "tariff_rate25_LTM" = "2025 LTM Tariffs",
                   "tariff_rate25_3M" = "2025 3M Tariffs",
                   gsub("_", " ", scenario_name))  # fallback for other scenarios

        # Create LaTeX table for this scenario
        create_latex_table(welfare_table,
                          paste0("welfare_effects ", scenario_name),
                          paste0("Welfare Effects by Country - ", good_name),
                          paste0("tab:welfare_", scenario_name),
                          paste0(output_dir, "/welfare_effects_", scenario_name, ".tex"),
                          labor_mobility = labor_mobility)
    }
    
    cat("✓ Welfare tables exported to:", output_dir, "\n")
    cat("  - Created", length(welfare_results), "scenario tables with countries as rows\n")
}

# Helper function to calculate trade-weighted average tariffs for a specific country
calculate_trade_weighted_tariffs <- function(tariffs, trade_shares, sector_shares, us_index, country_index, sectors) {
    country_tariffs <- 0
    ...
    for (k in 1:length(sectors)) {
        import_value <- trade_shares[country_index, us_index, k] * sector_shares[k]
        tariff_rate <- tariffs[country_index, us_index, k]
        
        total_imports <- total_imports + import_value
        weighted_tariff_sum <- weighted_tariff_sum + (tariff_rate * import_value)
    }
    
    if (total_imports > 0) {
        country_tariffs <- weighted_tariff_sum / total_imports
    }
