...
load_tariff_data <- function(tariff_file_path = NULL, country_groups = NULL, sector_groups = NULL, countries, sectors) {
    ...
    mutate(
      across(c(importer, exporter), ~ case_when(
        ...
        .x == "Canada" ~ "CAN",
        .x == "China" ~ "CHN",
        str_detect(.x, "European.*Union") ~ "EU",
        .x == "United Kingdom" ~ "GBR",
        .x == "India" ~ "IND",
        .x == "Japan" ~ "JPN",
        .x == "Mexico" ~ "MEX",
        str_detect(.x, "Rest.*World") ~ "RoW", 
        .x == "United States" ~ "USA",
        TRUE ~ .x
      ))
    )

    # Create 3D array for tariffs
    tariff_array <- data.frame(
      importer = rep(countries, each = n_countries * n_sectors),
      exporter = rep(rep(countries, each = n_sectors), times = n_countries),
      sector = rep(sectors, times = n_countries * n_countries),
      tariff_rate = 0  # Default to 0
    )
    
    # Fill in tariff rates from data
    for (i in 1:nrow(tariff_data)) {
      imp <- tariff_data$importer[i]
      exp <- tariff_data$exporter[i]
      sec <- tariff_data$sector[i]
      rate <- tariff_data$tariff_rate[i] / 100  # Convert percentage to decimal
      
      tariff_array$tariff_rate[tariff_array$importer == imp & 
                                 tariff_array$exporter == exp & 
                                 tariff_array$sector == sec] <- rate
    }
...
clean_all_data_cdk <- function(wiot_file_path, socioeco_file_path, exchange_rates_file_path,  tariff_file_path = NULL, target_year = 2011, country_groups = NULL, sector_groups = NULL) {
