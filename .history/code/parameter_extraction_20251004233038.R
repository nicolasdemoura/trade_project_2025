...
#' @param trade_flows_array 3D array of bilateral trade flows
#' @param total_expenditures Matrix of total expenditures by destination-sector
#' @param countries Vector of country names
#' @param sectors Vector of sector names
#' @return Data frame ready for CDK regression analysis
prepare_cdk_regression_data <- function(trade_flows_array, total_expenditures, countries, sectors) {
...
#' Estimate CDK technology parameters using regression approach
...
estimate_cdk_technology_params <- function(regression_data, theta = 6.53) {
    ...
    T_composite_matrix <- matrix(1, n_countries, n_sectors)
    ...
    # Handle missing values using softImpute for PCA-based matrix completion
    if (any(is.na(T_composite_matrix))) {
        # Use softImpute for matrix completion
        library(softImpute)
        
        # Apply softImpute to fill missing values
        fit <- softImpute(T_composite_matrix, rank.max = min(n_countries, n_sectors) - 1, 
                         lambda = 0, maxit = 100, trace.it = FALSE)
        
        # Extract completed matrix
        T_composite_matrix <- complete(T_composite_matrix, fit)
        
        # Ensure positive values for technology parameters
        T_composite_matrix[T_composite_matrix <= 0] <- 0.01
        
        cat("  - Used softImpute for missing value imputation\n")
        cat("  - Missing values filled:", sum(is.na(T_composite_matrix) | T_composite_matrix <= 0), "\n")
    }
...
get_technology_parameters <- function(processed_wiod, countries, sectors, theta = 6.53) {
    ...
    # Prepare regression data from building blocks
    ...
    cat("  - Regression R-squared:", round(summary(tech_estimation_results$regression_results)$r.squared, 4), "\n")
