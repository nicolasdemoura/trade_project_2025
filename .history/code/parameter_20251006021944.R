...
process_tariff_data <- function(raw_database) {
    ...
    # Update diagonal to zero (no tariffs on domestic trade)
    for (s in 1:n_sectors) {
        diag(tariff_array[, , s]) <- 0
    }

    # Update US tariffs from the other source
    for (i in 1:nrow(raw_database$us_tariff_scenarios$tariff_rate24)) {
        imp <- raw_database$us_tariff_scenarios$tariff_rate24$country[i]
        exp <- raw_database$us_tariff_scenarios$tariff_rate24$sector[i]
        sec <- raw_database$us_tariff_scenarios$tariff_rate24$[i]
        rate <- raw_database$us_tariff_scenarios$tariff_rate24$tariff_rate[i] / 100  # Convert percentage to decimal
    ...
    raw_database$us_tariff_scenarios$tariff_rate24
...
#' @return Array of tariff rates (origin x destination x sector)
create_tariff_scenario_array <- function(new_tariff_scenarios, scenario_name, countries, sectors, baseline_tariffs) {
    
    n_countries <- length(countries)
    n_sectors <- length(sectors)
    
    # Initialize tariff array 
