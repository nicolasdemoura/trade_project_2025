...
#' Solve CDK equilibrium system based on equilibrium conditions from body.tex
#' @param alpha_matrix Expenditure shares (countries x sectors)
#' @param beta_matrix Labor shares (countries x sectors)  
#' @param gamma_matrix Intermediate input shares (countries x sectors x sectors)
#' @param technology_matrix Technology parameters (countries x sectors)
#' @param theta Trade elasticity
#' @param tariffs Tariff rates (origin x destination x sectors)
#' @param labor_matrix Labor allocation (countries x sectors)
#' @param iceberg_costs Initial iceberg trade costs (origin x destination x sectors)
...
#' @return Solved equilibrium with all endogenous variables
solve_cdk_equilibrium <- function(alpha_matrix, beta_matrix, gamma_matrix, technology_matrix, 
                                 theta, tariffs, labor_matrix, iceberg_costs, 
                                 labor_mobility = TRUE, max_iterations = 1000, tolerance = 1e-6) {
    
    countries <- rownames(alpha_matrix)
    sectors <- colnames(alpha_matrix)
    n_countries <- nrow(alpha_matrix)
    n_sectors <- ncol(alpha_matrix)
    
    cat("Solving CDK equilibrium system...\n")
    ...
    # Iterative solution of equilibrium system
    for (iter in 1:max_iterations) {
        ...
        if (wage_diff < tolerance && price_diff < tolerance) {
...
#' Calibrate iceberg costs to match observed trade patterns
...
#' @param tariffs Tariff rates (origin x destination x sectors)
#' @param labor_matrix Labor allocation (countries x sectors)
#' @param labor_mobility Logical for labor mobility
#' @return Calibrated iceberg costs
calibrate_iceberg_costs <- function(observed_trade_shares, alpha_matrix, beta_matrix, gamma_matrix,
                                   technology_matrix, theta, tariffs, labor_matrix, labor_mobility = TRUE) {
    ...
    cat("Calibrating iceberg costs to match observed trade patterns...\n")
    
    # Initialize iceberg costs
    ...
    # Simple iterative approach to match trade shares
    for (iter in 1:100) {
        # Solve equilibrium with current iceberg costs
        equilibrium <- solve_cdk_equilibrium(
            alpha_matrix, beta_matrix, gamma_matrix, technology_matrix,
            theta, tariffs, labor_matrix, iceberg_costs, labor_mobility,
            max_iterations = 200, tolerance = 1e-4
        )
        
        # Update iceberg costs based on trade share differences
        # If predicted > observed, increase cost; if predicted < observed, decrease cost
            ...
            for (n in 1:n_countries) {
                for (k in 1:n_sectors) {
                    if (i != n) {  # Only adjust international trade costs
                        predicted <- equilibrium$trade_shares[i, n, k]
                        observed <- observed_trade_shares[i, n, k]
                        
                        if (!is.na(observed) && observed > 0) {
                            # Adjust iceberg cost inversely to trade share difference
        ...
        # Check convergence
        if (iter %% 20 == 0) {
            correlation <- cor(as.vector(equilibrium$trade_shares), as.vector(observed_trade_shares), 
                             use = "complete.obs")
            cat("  - Iteration", iter, "| Trade share correlation:", round(correlation, 4), "\n")
            
            if (correlation > 0.85) {  # Good enough fit
                break
            }
        }
    }
    ...
    cat("  ✓ Iceberg costs calibrated\n")
    
    return(iceberg_costs)
}
...
calibrate_model <- function(parameters, cleaned_data, labor_mobility = TRUE) {
    ...
    # Step 1: Calibrate iceberg costs to match observed trade patterns
    if (!is.null(cleaned_data$observed_trade_shares)) {
        cat("  - Calibrating iceberg costs to match observed trade patterns...\n")
        iceberg_costs <- calibrate_iceberg_costs(
        ...
        # Initialize with default costs if no observed data
        iceberg_costs <- array(1.5, dim = c(n_countries, n_countries, n_sectors))
        for (k in 1:n_sectors) {
            for (i in 1:n_countries) {
                iceberg_costs[i, i, k] <- 1.0
        ...
        dimnames(iceberg_costs) <- list(countries, countries, sectors)
        cat("  - Using default iceberg costs (no observed trade data)\n")
    }
    
    # Step 2: Solve final equilibrium system using proper equilibrium conditions from body.tex
    equilibrium <- solve_cdk_equilibrium(
        alpha_matrix = alpha_matrix,
        beta_matrix = beta_matrix,
        gamma_matrix = gamma_matrix,
    ...
    calibrated_parameters <- list(
        iceberg_costs = iceberg_costs  # Use initial iceberg costs (could be calibrated further)
    )
