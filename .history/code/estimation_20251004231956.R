...
#' Estimate technology parameters using CDK regression methodology
#' @param database Database containing WIOD trade data and socioeconomic data
#' @param theta Trade elasticity parameter
#' @return Matrix of estimated technology parameters (countries x sectors)
estimate_technology_parameters <- function(database, theta = 6.53) {
    
    cat("Estimating technology parameters using CDK regression methodology...\n")
    
    # Extract data components
    trade_flows_array <- database$trade_flows_array
    observed_trade_shares <- database$observed_trade_shares
    ...
    # Calculate expenditure matrix from trade flows
    ...
    regression_data <- prepare_cdk_regression_data(trade_flows_array, expenditure_matrix, countries, sectors)
    
    # Estimate technology parameters using CDK approach
    tech_estimation_results <- estimate_cdk_technology_params(regression_data, theta)
    
    # Extract technology matrix
    T_matrix <- tech_estimation_results$technology_composite
    ...
    cat("  - Matrix dimensions:", paste(dim(T_matrix), collapse = " x "), "\n")
    cat("  - Regression R-squared:", summary(tech_estimation_results$regression_results)$r.squared, "\n")
    
    return(list(
        technology_matrix = T_matrix,
        regression_results = tech_estimation_results$regression_results,
        estimation_details = tech_estimation_results
    ))
}

#' Prepare CDK regression data for technology parameter estimation
#' @param trade_flows_array 3D array of bilateral trade flows (origin x destination x sectors)
#' @param expenditure_matrix Matrix of total expenditure by destination and sector  
...
calibrate_multisector_baseline <- function(T_matrix, cleaned_data, theta = NULL, labor_mobility = TRUE) {
    ...
    baseline_equilibrium <- calibrate_equilibrium_from_trade_shares(
        ...
        baseline_tariffs = cleaned_data$tariffs,  # Pass tariff data
...
calibrate_equilibrium_from_trade_shares <- function(theta, T_matrix, labor_matrix, beta_matrix, gamma_matrix, expenditure_matrix, observed_trade_shares, baseline_tariffs = NULL, labor_mobility = FALSE) {
    ...
    objective_function <- function(params) {
        ...
        if (labor_mobility) {
            ...
            # Immobile labor: sector-specific wages (normalize only first country, first sector to 1)
            for (i in 1:n_countries) {
                for (k in 1:n_sectors) {
                    if (i == 1 && k == 1) {
                        wages[i, k] <- 1  # Numeraire: first country, first sector wage = 1
                    } else {
    ...
    n_wage_params <- if (labor_mobility) (n_countries - 1) else (n_countries * n_sectors - 1)  # Normalize only first country, first sector
    n_price_params <- n_countries * n_sectors
    ...
    if (labor_mobility) {
        ...
        for (i in 1:n_countries) {
            for (k in 1:n_sectors) {
                if (i == 1 && k == 1) {
                    wages_optimal[i, k] <- 1  # Numeraire: first country, first sector wage = 1
                } else {
    ...
    tariff_revenue <- calculate_tariff_revenue(trade_shares_optimal, tau_array_optimal, baseline_tariffs, expenditure_matrix, wages_optimal, n_countries, n_sectors)
    
    # Calculate real income including tariff revenue transfers  
    real_income <- numeric(n_countries)
    for (i in 1:n_countries) {
        # Basic real income (wages deflated by price level)
        total_labor <- sum(labor_matrix[i, ])
        if (total_labor > 0) {
            avg_wage <- sum(wages_optimal[i, ] * labor_matrix[i, ]) / total_labor
        } else {
        ...
        # Add tariff revenue as lump-sum transfer to consumers
        total_tariff_revenue <- sum(tariff_revenue[i, ])
        total_expenditure <- sum(expenditure_matrix[i, ])
        
        # Real income = (wage_income + tariff_revenue) / price_level
        real_income[i] <- (avg_wage * total_labor + total_tariff_revenue) / price_level
    }
    ...
    return(list(
        ...
        calibrated_trade_costs = tau_array_optimal,  # Total costs including tariffs
        base_trade_costs = tau_base_optimal,          # Base iceberg costs without tariffs
        baseline_tariffs = baseline_tariffs,          # Applied tariff rates
        tariff_revenue = tariff_revenue,             # Tariff revenue by country-sector
        gdp = gdp,
        welfare = welfare,                           # Basic welfare (wages/prices)
        real_income = real_income,                   # Welfare including tariff revenue
        ...
        tau_array = tau_array_optimal,
        T_matrix = T_matrix,
        theta_vec = rep(theta, n_sectors),
        beta_vec = rep(mean(beta_matrix), n_sectors),
        pure_technology = T_matrix,
        labor_matrix = labor_matrix,
        beta_matrix = beta_matrix
    ))
}
