...
create_io_matrices <- function(cleaned_data) {
    ...
    # Set expenditure matrix from expenditure data or GDP
    if (!is.null(cleaned_data$gdp_vector)) {
        ...
        # Use expenditure matrix if available
        if (!is.null(cleaned_data$expenditure_matrix)) {
...
calibrate_multisector_baseline <- function(cdk_estimation_results, cleaned_data, tau_array = NULL, theta = NULL, labor_mobility = FALSE) {
...
solve_equilibrium_cdk_multi_sector <- function(theta, T_matrix, tau_array, labor_matrix, beta_matrix, gamma_matrix, expenditure_matrix, labor_mobility = FALSE, max_iter = 1000, tol = 1e-8) {
