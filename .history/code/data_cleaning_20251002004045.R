...
clean_wiod_data_cdk <- function(wiot_file_path, target_year = 2011) {
  ...
  if(!file.exists(paste0(wiot_file_path, "_", target_year, ".csv"))) {
    # Load WIOD data from file
    wiot_raw <- read_dta(wiot_file_path)
    
    wiot_2011 <- wiot_raw[wiot_raw$year == 2011, ]
    
    # Export 2011 data for inspection
    readr::write_csv(wiot_2011, paste0(wiot_file_path, "_2011.csv"))
  } else {
    # Load WIOD data from pre-saved 2011 CSV
    wiot_2011 <- readr::read_csv(paste0(wiot_file_path, "_", target_year, ".csv"))
  }
...
clean_socioeconomic_data_cdk <- function(socioeco_file_path, countries, sectors, target_year = 2011) {
  ...
  # Find common countries and those that need to be aggregated as RoW
  common_countries <- intersect(countries, countries_with_2011_data)
  
  # Countries in socioeco but not in WIOD countries - aggregate as "RoW"
  row_countries <- setdiff(countries_with_2011_data, countries)
  
  if (length(row_countries) > 0) {
    # Aggregate RoW countries data
    row_data <- socioeco_data %>%
      filter(Country %in% row_countries, !is.na(.data[[year_col]])) %>%
      group_by(Variable, Description) %>%
      summarise(!!year_col := sum(.data[[year_col]], na.rm = TRUE), .groups = 'drop') %>%
      mutate(Country = "RoW")
    
    # Add RoW to the dataset
    socioeco_data <- bind_rows(socioeco_data, row_data)
    common_countries <- c(common_countries, "RoW")
  }
...
clean_all_data_cdk <- function(wiot_file_path, socioeco_file_path, target_year = 2011) {
  ...
  wiod_cleaned_initial <- clean_wiod_data_cdk(wiot_file_path, target_year = target_year)

  initial_countries <- wiod_cleaned_initial$countries
  sectors <- wiod_cleaned_initial$sectors

  socio_cleaned <- clean_socioeconomic_data_cdk(socioeco_file_path, initial_countries, sectors, target_year)
