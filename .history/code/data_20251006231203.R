...
create_latex_table <- function(data, filename, caption, label, output_dir = "tables") {
  ...
  if (is.numeric(data)) {
    ...
    # Apply color gradient from orange (small) to blue (large)
    for (i in 1:nrow(data)) {
      for (j in 1:ncol(data)) {
      if (!is.na(data[i, j])) {
        # Normalize value to 0-1 scale
        normalized <- (data[i, j] - min_val) / (max_val - min_val)
        
        # Create direct gradient from orange to blue
        red <- round(255 * (1 - normalized))      # 255->0 (orange to blue)
        green <- round(165 * (1 - normalized))    # 165->0 (orange component)
        blue <- round(255 * normalized)           # 0->255 (blue component)
        
        data_colored[i, j] <- paste0("\\cellcolor[RGB]{", 
                     red, ",", green, ",", blue, "}",
                     sprintf("%.3f", data[i, j]))
      }
      }
