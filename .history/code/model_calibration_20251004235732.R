...
calculate_model_trade_shares_clean <- function(technology_matrix, iceberg_costs, unit_costs, tariffs, theta) {
...
estimate_cdk_method_of_moments <- function(database, parameters, labor_mobility = TRUE) {
    ...
    # Create helper function to unpack parameters for optimization
    unpack_params <- function(params) {
        param_idx <- 1
        
        # Unpack iceberg costs
        iceberg_costs <- array(1.0, dim = c(n_countries, n_countries, n_sectors))  # Domestic = 1
        for (k in 1:n_sectors) {
            for (i in 1:n_countries) {
                for (j in 1:n_countries) {
                    if (i != j) {
                        iceberg_costs[i, j, k] <- params[param_idx]
                        param_idx <- param_idx + 1
                    }
                }
            }
        ...
        return(list(iceberg_costs = iceberg_costs, wages = wages, prices = prices))
    }
    ...
    optimization_result <- optim(
        ...
        control = list(
            ...
            REPORT = 50      # Report every 50 iterations
        )
    )
    ...
    final_params <- unpack_params(optimization_result$par)
    iceberg_costs <- final_params$iceberg_costs
    wages <- final_params$wages
    prices <- final_params$prices
