...
estimate_cdk_method_of_moments <- function(database, parameters, labor_mobility = TRUE) {
    ...
    # 1. Initialize bilateral costs d_ni (start at 1.2, exclude domestic = 1)
    for (i in 1:n_countries) {
        for (j in 1:n_countries) {
            if (i != j) {
                initial_params[param_idx] <- 1.2
                param_idx <- param_idx + 1
            }
        }
    }
    ...
    for (i in 1:n_countries) {
        for (j in 1:n_countries) {
            if (i != j) {
                bilateral_costs[i, j] <- final_params[param_idx]
                param_idx <- param_idx + 1
            }
        }
    }
    ...
    for (i in 1:n_countries) {
        for (k in 1:n_sectors) {
            if (!(i == 1 && k == 1)) {
                importer_effects[i, k] <- final_params[param_idx]
                param_idx <- param_idx + 1
            }
        }
    }
    ...
    for (j in 1:n_countries) {
        for (k in 1:n_sectors) {
            if (!(j == 1 && k == 1)) {
                exporter_effects[j, k] <- final_params[param_idx]
                param_idx <- param_idx + 1
            }
        }
    }
    ...
    if (labor_mobility) {
        ...
        for (i in 2:n_countries) {
            country_wages[i] <- final_params[param_idx]
            param_idx <- param_idx + 1
        }
