...
gdp_plot_data <- data.frame(
  observed_gdp = log(as.vector(processed_database$gdp_data)),
  predicted_gdp = log(as.vector(equilibrium_solution$outcome_variables$real_income))
)
...
counterfactual_parameters <- model_parameters
counterfactual_parameters$tariffs <- counterfactual_tariffs

# Create counterfactual database (without observed trade shares for faster convergence)
counterfactual_database <- processed_database
counterfactual_database$bilateral_trade_shares <- NULL  # No need to match observed in counterfactual

counterfactual_solution <- solve_cdk_equilibrium(
    database = counterfactual_database,
