...
cleaned_data <- clean_all_data_cdk(wiot_file_path, socioeco_file_path, exchange_rates_file_path, target_year, country_groups, sector_groups)
...
trade_shares_plot_data <- data.frame(
  ...
  predicted_trade_shares = as.vector(baseline_results$trade_shares_optimal)
)
...
gdp_plot_data <- data.frame(
  observed_log_gdp = log(cleaned_data$gdp_data$_2009),
  predicted_log_gdp = log(baseline_results$gdp)
)
...
gg_scatterplot(data = gdp_plot_data,
         ...
         subtitle = paste("Reference Year:", target_year),
         x_label = "Countries",
         y_label = "Log GDP",
         out_dir = "figures", 
         out_name = "gdp_distribution",
         width = 16,
         height = 9,
         point_color = "#0066CC",
         point_size = 3,
