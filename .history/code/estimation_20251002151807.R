...
calibrate_equilibrium_from_trade_shares <- function(theta, T_matrix, labor_matrix, beta_matrix, gamma_matrix, expenditure_matrix, observed_trade_shares, labor_mobility = FALSE) {
    ...
    objective_function <- function(params) {
        ...
        n_wage_params <- if (labor_mobility) (n_countries - 1) else (n_countries - 1) * n_sectors  # Exclude numeraire
            ...
            for (i in 1:n_countries) {
                for (j in 1:n_countries) {
                    if (i != j) {  # Only non-domestic costs are optimized
                        tau_array[i, j, s] <- exp(params[param_idx])
        ...
        return(objective_value + 0.01 * price_penalty)  # Small weight on price consistency
    }
    ...
    # Extract optimal iceberg costs
    tau_array_optimal <- array(1, dim = c(n_countries, n_countries, n_sectors))
    for (s in 1:n_sectors) {
        for (i in 1:n_countries) {
            for (j in 1:n_countries) {
                if (i != j) {
                    tau_array_optimal[i, j, s] <- exp(optimal_params[param_idx])
                    param_idx <- param_idx + 1
                }
            }
        }
    ...
    return(list(
        ...
        method = "optimization_based_calibration"
    ))
}
