...
country_groups <- list(
  "EU" = EU_COUNTRIES,
  "RoW" = c("AUS","IDN","KOR","RUS","TUR","TWN","RoW", "IDN", "JPN", "GBR", "CAN", "MEX")
)

# Define sector groups - mapping from WIOD codes to aggregated sectors
# Includes both original codes (AtB, C, 15t16, etc.) and alternative codes (c1, c2, c3, etc.)
sector_groups <- list(
  "Food" = c("AtB", "c1", "15t16", "c3"),  # Agriculture + Food products
  "Textiles" = c("17t18", "c4", "19", "c5"),  # Textiles + Leather
  "Wood" = c("20", "c6"),  # Wood products
  "Paper" = c("21t22", "c7"),  # Paper products
  "Fuel" = c("C", "c2", "23", "c8"),  # Mining + Petroleum products
  "Chemicals" = c("24", "c9"),  # Chemicals
  "Plastic" = c("25", "c10"),  # Rubber and plastic
  "Minerals" = c("26", "c11"),  # Non-metallic minerals
  "Metals" = c("27t28", "c12"),  # Basic metals and fabricated metal
  "Machinery" = c("29", "c13"),  # Machinery
  "Electrical" = c("30t33", "c14"),  # Electrical equipment
  "Transport" = c("34t35", "c15", "60", "c23", "61", "c24", "62", "c25"),  # Transport equipment + Transport services
  "Misc.Manuf." = c("36t37", "c16"),  # Other manufacturing
  "Services" = c("E", "c17", "F", "c18", "50", "c19", "51", "c20", "52", "c21", "H", "c22", 
                "63", "c26", "64", "c27", "J", "c28", "70", "c29", "71t74", "c30", 
                "L", "c31", "M", "c32", "N", "c33", "O", "c34", "P", "c35")  # All services
)

# Load and clean the data with country and sector aggregation applied BEFORE cleaning
cleaned_data <- clean_all_data_cdk(wiot_file_path, socioeco_file_path, target_year, country_groups, sector_groups)
...
#
# CALIBRATION METHOD:
# - Uses OPTIMIZATION-BASED CALIBRATION to match observed trade shares
# - Optimizes iceberg costs (τ_ij), wages (w_ik), and prices (p_nk) simultaneously
# - Minimizes sum of squared differences between predicted and observed trade shares  
# - Finds the best-fit parameters that reproduce observed trade patterns
# - Much more accurate than iterative methods or direct matching
#
...
baseline_results <- calibrate_multisector_baseline(T_matrix = T_matrix, 
                                                   ...
                                                   theta = theta,
                                                   labor_mobility = TRUE)  # Change to FALSE for immobile labor
