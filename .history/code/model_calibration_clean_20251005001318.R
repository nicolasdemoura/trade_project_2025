...
moments_objective_function <- function(params, observed_trade_shares, technology_matrix, tariffs, 
    ...
    n_importer <- n_countries * n_sectors           # Importer effects d_ik^importer 
    n_exporter <- n_countries * n_sectors           # Exporter effects d_jk^exporter
    ...
    for (i in 1:n_countries) {
        for (j in 1:n_countries) {
            if (i != j) {
                bilateral_costs[i, j] <- params[param_idx]
    ...
    for (i in 1:n_countries) {
        for (k in 1:n_sectors) {
            if (!(i == 1 && k == 1)) {  # Skip normalization
                importer_effects[i, k] <- params[param_idx]
                param_idx <- param_idx + 1
            }
        }
    }
    ...
    for (j in 1:n_countries) {
        for (k in 1:n_sectors) {
            if (!(j == 1 && k == 1)) {  # Skip normalization
                exporter_effects[j, k] <- params[param_idx]
                param_idx <- param_idx + 1
            }
        }
    }
    ...
    if (labor_mobility) {
        ...
        for (i in 2:n_countries) {  # Skip normalized country
            country_wages[i] <- params[param_idx]
            param_idx <- param_idx + 1
        }
    ...
    # Calculate expenditures and incomes for trade balance conditions
    total_income <- numeric(n_countries)
    expenditures <- matrix(0, n_countries, n_sectors)
    
    # Calculate labor income
    for (n in 1:n_countries) {
        total_income[n] <- sum(wages[n, ] * labor_matrix[n, ])
    }
    
    # Initial expenditures (will be updated iteratively)
    for (n in 1:n_countries) {
        for (k in 1:n_sectors) {
    ...
    # Iteratively solve income-expenditure system
    for (iter in 1:20) {
        total_income_old <- total_income
        
        # Update total income with tariff revenue
        for (n in 1:n_countries) {
            labor_income <- sum(wages[n, ] * labor_matrix[n, ])
            tariff_revenue <- 0
            for (k in 1:n_sectors) {
                for (i in 1:n_countries) {
                    tariff_revenue <- tariff_revenue + tariffs[i, n, k] * model_trade_shares[i, n, k] * expenditures[n, k]
                }
            }
        ...
        # Check convergence
        if (max(abs(total_income - total_income_old)) < 1e-6) {
            break
        }
    }
    ...
    if (labor_mobility) {
        ...
        for (i in 1:n_countries) {
            ...
            for (k in 1:n_sectors) {
                for (n in 1:n_countries) {
                    lhs <- lhs + model_trade_shares[i, n, k] * alpha_matrix[n, k] * total_income[n]
                }
            }
            
            rhs <- 0  # Right-hand side: labor income + tariff revenue
            for (k in 1:n_sectors) {
                rhs <- rhs + wages[i, k] * labor_matrix[i, k]
                for (n in 1:n_countries) {
                    rhs <- rhs + tariffs[i, n, k] * model_trade_shares[i, n, k] * alpha_matrix[n, k] * total_income[n]
                }
            }
    ...
    # Combine trade share matching and trade balance violations
    objective_value <- trade_share_objective + 1000 * trade_balance_violations
    
    # Add penalty for extreme parameter values to ensure stability
    penalty <- 0
    
    # Bilateral cost penalties
    if (any(bilateral_costs < 0.8, na.rm = TRUE) | any(bilateral_costs > 5, na.rm = TRUE)) {
        penalty <- penalty + 1000
    }
    
    # Importer/exporter effect penalties
    if (any(importer_effects < 0.5, na.rm = TRUE) | any(importer_effects > 3, na.rm = TRUE)) {
        penalty <- penalty + 1000
    }
    if (any(exporter_effects < 0.5, na.rm = TRUE) | any(exporter_effects > 3, na.rm = TRUE)) {
        penalty <- penalty + 1000
    }
    
    # Wage and price penalties
...
estimate_cdk_method_of_moments <- function(database, parameters, labor_mobility = TRUE) {
    ...
    # Parameters to estimate: iceberg_costs (excluding domestic=1), wages, prices
    n_iceberg <- n_countries * (n_countries - 1) * n_sectors  # Exclude domestic trade costs
    n_wages <- n_countries * n_sectors - 1  # Exclude normalized wage 
    n_prices <- n_countries * n_sectors
    n_params <- n_iceberg + n_wages + n_prices
    
    cat("  - Parameters to estimate:", n_params, "(iceberg:", n_iceberg, ", wages:", n_wages, ", prices:", n_prices, ")\n")
    
    # Initial parameter values
    initial_params <- numeric(n_params)
    
    # Initialize iceberg costs (start at 1.5 for all bilateral pairs, exclude domestic)
    param_idx <- 1
    for (k in 1:n_sectors) {
        for (i in 1:n_countries) {
            for (j in 1:n_countries) {
                if (i != j) {  # Skip domestic pairs
                    initial_params[param_idx] <- 1.5
                    param_idx <- param_idx + 1
                }
            }
        }
    ...
    # Initialize wages (start at 1, exclude normalized wage w[1,1]=1)
    for (i in 1:n_countries) {
        for (k in 1:n_sectors) {
            if (!(i == 1 && k == 1)) {  # Skip normalized wage
                initial_params[param_idx] <- 1.0
                param_idx <- param_idx + 1
            }
        }
    }
    
    # Initialize prices (start at 1)
    for (i in 1:n_countries) {
        for (k in 1:n_sectors) {
            initial_params[param_idx] <- 1.0
            param_idx <- param_idx + 1
    ...
    iceberg_costs <- array(1.0, dim = c(n_countries, n_countries, n_sectors))
    param_idx <- 1
    for (k in 1:n_sectors) {
        for (i in 1:n_countries) {
            for (j in 1:n_countries) {
                if (i != j) {
                    iceberg_costs[i, j, k] <- final_params[param_idx]
                    param_idx <- param_idx + 1
                }
            }
        }
    ...
    for (i in 1:n_countries) {
        for (k in 1:n_sectors) {
            if (!(i == 1 && k == 1)) {
                wages[i, k] <- final_params[param_idx]
                param_idx <- param_idx + 1
            }
        }
    }
    ...
    for (i in 1:n_countries) {
        for (k in 1:n_sectors) {
            prices[i, k] <- final_params[param_idx]
            param_idx <- param_idx + 1
        }
    }
    ...
    # Calculate total income: Y_n = ∑w_nk*L_nk + ∑∑τ_nik*π_nik*X_nk (Income Identity)
    # Solve iteratively since income and expenditures depend on each other
    for (iter_income in 1:50) {
        total_income_old <- total_income
        
        # Calculate labor income component
        for (n in 1:n_countries) {
            labor_income <- sum(wages[n, ] * labor_matrix[n, ])
            total_income[n] <- labor_income  # Start with labor income
        }
        
        # Update expenditures: X_nk = α_nk * Y_n
        for (n in 1:n_countries) {
            for (k in 1:n_sectors) {
                expenditures[n, k] <- alpha_matrix[n, k] * total_income[n]
            }
    ...
    return(list(
        ...
        iceberg_costs = iceberg_costs,  # Include estimated iceberg costs
...
#' Main solve function (wrapper for method of moments estimation)
solve_cdk_equilibrium <- function(database, parameters, labor_mobility = TRUE, max_iter = 1000, tolerance = 1e-1) {
    return(estimate_cdk_method_of_moments(database, parameters, labor_mobility))
}
...
calibrate_model <- function(parameters, cleaned_data, labor_mobility = TRUE) {
    ...
    # 1. PARAMETERS: Estimated iceberg trade costs
    calibrated_parameters <- list(
        iceberg_costs = equilibrium$iceberg_costs  # Estimated via method of moments
    )
