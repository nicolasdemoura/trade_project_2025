...
process_tariff_data <- function(raw_database) {
    ...
    for (i in 1:nrow(raw_database$us_tariff_scenarios)) {
        imp <- "USA"
        exp <- raw_database$us_tariff_scenarios$country[i]
        sec <- raw_database$us_tariff_scenarios$sector[i]
        rate <- raw_database$us_tariff_scenarios$tariff_rate24[i] 
