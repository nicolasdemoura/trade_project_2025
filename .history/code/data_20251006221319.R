...
export_parameter_tables <- function(processed_database, model_parameters, output_dir = "tables"){
  ...
  alpha_xtable <- xtable(model_parameters$alpha, caption = "Expenditure Shares ($\\alpha$)", label = "tab:alpha", align = paste0("l", paste(rep("c", ncol(model_parameters$alpha)), collapse = "")))
  print(alpha_xtable, file = file.path(output_dir, "alpha_table.tex"), include.rownames = TRUE, caption.placement = "top", 
    sanitize.text.function = identity,
    tabular.environment = "longtable",
    floating = FALSE)

  beta_xtable <- xtable(model_parameters$beta, caption = "Labor Shares ($\\beta$)", label = "tab:beta", align = paste0("l", paste(rep("c", ncol(model_parameters$beta)), collapse = "")))
  print(beta_xtable, file = file.path(output_dir, "beta_table.tex"), include.rownames = TRUE, caption.placement = "top", 
    sanitize.text.function = identity,
    tabular.environment = "longtable",
    floating = FALSE)
  ...
  for(j in 1:dim(model_parameters$gamma)[1]) {
    ...
    gamma_xtable <- xtable(gamma_matrix, 
          ...
          label = paste0("tab:gamma_", country), align = paste0("l", paste(rep("c", ncol(gamma_matrix)), collapse = "")))
    print(gamma_xtable, 
      file = file.path(output_dir, paste0("gamma_table_", country, ".tex")), 
  ...
  T_xtable <- xtable(model_parameters$technology, caption = "Technology Parameters", label = "tab:technology", align = paste0("l", paste(rep("c", ncol(model_parameters$technology)), collapse = "")))
  print(T_xtable, file = file.path(output_dir, "technology_table.tex"), include.rownames = TRUE, caption.placement = "top", 
    sanitize.text.function = identity,
    tabular.environment = "longtable",
    floating = FALSE)

  # Export tariff tables for each country using xtable  
  for(i in 1:dim(processed_database$tariffs)[1]) {
    ...
    tariff_xtable <- xtable(tariff_vector, 
            caption = paste0("Tariff Rates - ", country), 
            label = paste0("tab:tariffs_", country), align = paste0("l", paste(rep("c", ncol(tariff_vector)), collapse = "")))
    print(tariff_xtable, 
      file = file.path(output_dir, paste0("tariff_table_", country, ".tex")),
      include.rownames = TRUE, 
      caption.placement = "top", 
      sanitize.text.function = identity,
      tabular.environment = "longtable",
