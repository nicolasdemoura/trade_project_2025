...
moments_objective_function <- function(params, observed_trade_shares, technology_matrix, tariffs, 
    ...
    n_importer <- n_countries * n_sectors           # Importer effects d_ik^importer (exclude normalization)
    n_exporter <- n_countries * n_sectors           # Exporter effects d_jk^exporter (exclude normalization)
    ...
    objective_value <- trade_share_objective + 100*(trade_balance_violations * 1e-9 + price_consistency_violations)
...
solve_cdk_equilibrium <- function(database, parameters, labor_mobility = TRUE) {
    ...
    n_importer <- n_countries * n_sectors       # Importer effects (exclude normalization)
    n_exporter <- n_countries * n_sectors       # Exporter effects (exclude normalization)
    n_iceberg_total <- n_bilateral + n_importer + n_exporter
...
# Export calibrated iceberg costs to LaTeX tables
export_iceberg_tables <- function(equilibrium_solution, countries, sectors, output_dir = "tables") {
    ...
    # Extract iceberg costs from equilibrium solution
    iceberg_costs <- equilibrium_solution$parameters$iceberg_costs
    bilateral_costs <- equilibrium_solution$parameters$bilateral_costs
    importer_effects <- equilibrium_solution$parameters$importer_effects
    exporter_effects <- equilibrium_solution$parameters$exporter_effects

    # Export full iceberg costs matrix for each sector
    for(k in 1:dim(iceberg_costs)[3]) {
    sector <- dimnames(iceberg_costs)[[3]][k]
    iceberg_matrix <- iceberg_costs[,,k]
    colnames(iceberg_matrix) <- countries
    rownames(iceberg_matrix) <- countries

    iceberg_xtable <- xtable(iceberg_matrix,
                            caption = paste0("Iceberg Trade Costs ($d_{nik}$) - ", sector),
                            label = paste0("tab:iceberg_", sector),
                            align = paste0("l", paste(rep("c", ncol(iceberg_matrix)), collapse = "")))
    print(iceberg_xtable,
            file = file.path(output_dir, paste0("iceberg_costs_table_", sector, ".tex")),
    ...
    rownames(bilateral_costs) <- countries
    colnames(bilateral_costs) <- countries
    # Export bilateral costs (d_ni components)
    bilateral_xtable <- xtable(bilateral_costs,
                            caption = "Bilateral Trade Costs ($d_{ni}$)",
                            label = "tab:bilateral_costs",
                            align = paste0("l", paste(rep("c", ncol(bilateral_costs)), collapse = "")))
    print(bilateral_xtable,
        file = file.path(output_dir, "bilateral_costs_table.tex"),
        include.rownames = TRUE,
        caption.placement = "top",
        sanitize.text.function = identity,
        tabular.environment = "longtable",
    ...
    rownames(importer_effects) <- countries
    colnames(importer_effects) <- sectors   
    # Export importer effects
    importer_xtable <- xtable(importer_effects,
                            caption = "Importer Fixed Effects ($d_{ik}^{importer}$)",
                            label = "tab:importer_effects",
                            align = paste0("l", paste(rep("c", ncol(importer_effects)), collapse = "")))
    print(importer_xtable,
        file = file.path(output_dir, "importer_effects_table.tex"),
        include.rownames = TRUE,
        caption.placement = "top",
        sanitize.text.function = identity,
        tabular.environment = "longtable",
    ...
    rownames(exporter_effects) <- countries
    colnames(exporter_effects) <- sectors
    # Export exporter effects  
    exporter_xtable <- xtable(exporter_effects,
                            caption = "Exporter Fixed Effects ($d_{jk}^{exporter}$)",
                            label = "tab:exporter_effects",
                            align = paste0("l", paste(rep("c", ncol(exporter_effects)), collapse = "")))
    print(exporter_xtable,
        file = file.path(output_dir, "exporter_effects_table.tex"),
        include.rownames = TRUE,
        caption.placement = "top",
        sanitize.text.function = identity,
        tabular.environment = "longtable",
...
# Export calibrated iceberg costs to LaTeX tables
export_iceberg_tables <- function(equilibrium_solution, countries, sectors, output_dir = "tables", labor_mobility = TRUE) {
  ...
  create_latex_table(iceberg_matrix,
            paste0("iceberg_costs_table_", sector),
            paste0("Iceberg Trade Costs ($d_{nik}$) - ", sector),
            paste0("tab:iceberg_", sector),
            paste0(output_dir, "/iceberg"),
            labor_mobility)
  }

  # Set row and column names for remaining tables
  rownames(bilateral_costs) <- countries
  colnames(bilateral_costs) <- countries
  rownames(importer_effects) <- countries
  colnames(importer_effects) <- sectors   
  ...
  # Export bilateral costs
  create_latex_table(bilateral_costs,
          "bilateral_costs_table",
          "Bilateral Trade Costs ($d_{ni}$)",
          "tab:bilateral_costs",
          paste0(output_dir, "/iceberg"),
          labor_mobility)

  # Export importer effects
  create_latex_table(importer_effects,
          "importer_effects_table",
          "Importer Fixed Effects ($d_{ik}^{importer}$)",
          "tab:importer_effects",
          paste0(output_dir, "/iceberg"),
          labor_mobility)

  # Export exporter effects
  create_latex_table(exporter_effects,
          "exporter_effects_table",
          "Exporter Fixed Effects ($d_{jk}^{exporter}$)",
          "tab:exporter_effects",
          paste0(output_dir, "/iceberg"),
          labor_mobility)
}
