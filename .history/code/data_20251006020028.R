...
#' @param new_tariff_file_path Optional path to new HTS tariff CSV file
...
load_and_clean_raw_data <- function(wiot_file_path, socioeco_file_path, exchange_rates_file_path, 
                                   tariff_file_path = NULL, new_tariff_file_path = NULL, target_year = 2011, 
    ...
    new_tariff_scenarios <- NULL
    if (!is.null(new_tariff_file_path) && file.exists(new_tariff_file_path)) {
        source("new_tariff_processor.R")
        new_tariff_scenarios <- process_new_tariff_dataset(new_tariff_file_path)
    }
    ...
    raw_database <- list(
        ...
        tariff_data = tariff_data,
        new_tariff_scenarios = new_tariff_scenarios,
    ...
    cat("  - New tariff scenarios available:", !is.null(new_tariff_scenarios), "\n")
