...
load_tariff_data <- function(tariff_file_path = NULL, target_year = 2011, country_groups = NULL, sector_groups = NULL, countries, sectors) {
    ...
    for (i in 1:nrow(tariff_data)) {
        importer <- tariff_data$importer[i]
        exporter <- tariff_data$exporter[i]
        sector <- tariff_data$sector[i]
        tariff_rate <- tariff_data$tariff_rate[i]

        if (importer %in% countries && exporter %in% countries && sector %in% sectors) {
            tariff_array[importer, exporter, sector] <- tariff_rate
        }
    }
