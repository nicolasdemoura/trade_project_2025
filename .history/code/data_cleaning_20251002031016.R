...
clean_socioeconomic_data_cdk <- function(socioeco_file_path, target_year = 2011) {
    ...
    employment_data <- socioeco_data %>%
        ...
        rename(value_2011 = as.numeric(all_of(year_col)))
    ...
    socioeco_data$value_2011 <- as.numeric(socioeco_data$value_2011)
...
clean_all_data_cdk <- function(wiot_file_path, socioeco_file_path, target_year = 2011) {
    ...
    # Filter WIOD data to common countries
    country_indices <- which(wiod_cleaned$countries %in% common_countries)
    filtered_trade_flows <- wiod_cleaned$trade_flows_array[country_indices, country_indices, ]
    
    # Aggregate EU countries in trade flows
    # First aggregate origin countries (dimension 1)
    eu_agg_origin <- aggregate_eu_countries(filtered_trade_flows, common_countries, dimension = 1)
    # Then aggregate destination countries (dimension 2)  
    eu_agg_both <- aggregate_eu_countries(eu_agg_origin$aggregated_data, common_countries, dimension = 2)
    
    trade_flows_array <- eu_agg_both$aggregated_data
    countries <- eu_agg_both$new_countries
    
    # Filter and aggregate socioeconomic data to match new country list
    # For EU countries, we need to aggregate labor matrix and GDP
    socio_countries <- socio_cleaned$countries
    eu_indices_socio <- which(socio_countries %in% EU_COUNTRIES)
    non_eu_indices_socio <- which(!socio_countries %in% EU_COUNTRIES & socio_countries %in% common_countries)
    
    # Aggregate labor matrix
    if (length(eu_indices_socio) > 0) {
        eu_labor <- colSums(socio_cleaned$labor_matrix[eu_indices_socio, , drop = FALSE])
        labor_matrix <- rbind(eu_labor, socio_cleaned$labor_matrix[non_eu_indices_socio, , drop = FALSE])
        rownames(labor_matrix) <- countries
    } else {
        labor_matrix <- socio_cleaned$labor_matrix[non_eu_indices_socio, , drop = FALSE]
        rownames(labor_matrix) <- countries[-1]  # Exclude EU if no EU countries in socio data
    }
    
    # Aggregate GDP data
    if (length(eu_indices_socio) > 0) {
        eu_gdp <- sum(socio_cleaned$gdp_vector[eu_indices_socio])
        gdp_vector <- c(eu_gdp, socio_cleaned$gdp_vector[non_eu_indices_socio])
        names(gdp_vector) <- countries
    } else {
        gdp_vector <- socio_cleaned$gdp_vector[non_eu_indices_socio]
        names(gdp_vector) <- countries[-1]
    }
