...
create_latex_table <- function(data, filename, caption, label, output_dir = "tables") {
  ...
  # Apply color gradient from red (small) to blue (large)
  for (i in 1:nrow(data)) {
    ...
    if (!is.na(data[i, j])) {
      ...
      # Create color gradient: red for low values, blue for high values
      if (normalized < 0.5) {
      # Red to orange gradient for lower half
      ...
      # Orange to blue gradient for upper half  
      intensity <- (normalized - 0.5) * 2
      color_code <- paste0("\\textcolor[RGB]{", 
            round(255 * (1 - intensity)), ",", 
            round(165 * (1 - intensity)), ",", 
            round(255 * intensity), "}")
      }
  ...
  full_path <- file.path(output_dir, paste0(filename, ".tex"))
