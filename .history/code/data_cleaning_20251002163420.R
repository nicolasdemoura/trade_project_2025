...
clean_socioeconomic_data_cdk <- function(socioeco_file_path, target_year = 2011, country_groups = NULL, sector_groups = NULL) {
    ...
    gdp_data <- socioeco_data %>%
      ...
      rename(gdp = paste0("_", target_year))
    ...
    if (!is.null(country_groups)) {
      ...
      gdp_data <- aggregate_socioeco_before_cleaning(gdp_data, country_groups)
    }
