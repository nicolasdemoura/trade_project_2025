###############################################################################
# Topic: Welfare and tariffs: a Quantitative Analysis
# Goal: Main script to run the CDK model with iceberg cost estimation and counterfactual analysis
# Keywords: International Trade, Structural Model, Tariffs, Welfare Analysis
...
export_parameter_tables(processed_database, model_parameters, output_dir = "tables")

###############################################################################
# TASK 3: MODEL CALIBRATION - Mobile Labor
###############################################################################

cat("\n=== TASK 3a: MODEL CALIBRATION - Mobile Labor ===\n")
...
if (equilibrium_solution$summary$convergence_achieved) {
    ...
    cat("  - Trade shares π:", paste(dim(equilibrium_solution$model_solution$trade_shares), collapse = " x "), "\n")
    cat("  - Unit costs c:", paste(dim(equilibrium_solution$model_solution$unit_costs), collapse = " x "), "\n")
    cat("  - Price indices P:", paste(dim(equilibrium_solution$model_solution$prices), collapse = " x "), "\n")
    cat("  - Expenditures X:", paste(dim(equilibrium_solution$model_solution$expenditures), collapse = " x "), "\n")
    cat("  - Income Y:", length(equilibrium_solution$model_solution$total_income), "countries\n")
    cat("  - Wages w:", paste(dim(equilibrium_solution$model_solution$wages), collapse = " x "), "\n")
    cat("  - Iceberg costs d:", paste(dim(equilibrium_solution$calibrated_parameters$iceberg_costs), collapse = " x "), "\n")
} else {
...
gg_scatterplot(data = trade_shares_plot_data,
         ...
         x_label = "Observed Trade Shares",
         y_label = "Predicted Trade Shares",
         out_dir = "figures",
         out_name = "trade_flows_fit_mobile",
         width = 9,
         height = 9,
         xlim = c(0,1),
         ylim = c(0,1),
...
# Export iceberg trade costs tables
export_iceberg_tables(equilibrium_solution, 
...
cat("\n=== TASK 4a: COUNTERFACTUAL ANALYSIS ===\n")
welfare_analysis <- run_multiple_counterfactual_scenarios(baseline_equilibrium = equilibrium_solution,  
                                                          database = processed_database,
                                                          parameters = model_parameters,
...
for (scenario in names(welfare_analysis)) {
    cat("  - Scenario:", scenario, "\n")
    print(welfare_analysis[[scenario]]$welfare_analysis)
}

export_welfare_tables(welfare_results = welfare_analysis, 
                      baseline_equilibrium = equilibrium_solution, 
                      database = processed_database, 
                      output_dir = "tables", labor_mobility = TRUE)
...
), file = "output/cdk_model_results_mobile.rds")


###############################################################################
# TASK 3: MODEL CALIBRATION - immobile Labor
###############################################################################

cat("\n=== TASK 3b: MODEL CALIBRATION - immobile Labor ===\n")
...
gg_scatterplot(data = trade_shares_plot_data,
         ...
         y_label = "Predicted Trade Shares",
         out_dir = "figures",
         out_name = "trade_flows_fit_immobile",
         width = 9,
         height = 9,
         xlim = c(0,1),
         ylim = c(0,1),
...
export_welfare_tables(welfare_results = welfare_analysis, 
                      baseline_equilibrium = equilibrium_solution, 
                      database = processed_database, 
                      output_dir = "tables", labor_mobility = FALSE)
...
saveRDS(list(
    ...
    equilibrium_solution = equilibrium_solution,
    welfare_analysis = welfare_analysis
), file = "output/cdk_model_results_immobile.rds")
