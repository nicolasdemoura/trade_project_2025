###############################################################################
# Counterfactual Analysis: Trump Tariff Impact on Global Welfare
...
###############################################################################
# Define Trump Tariff Scenarios
###############################################################################

#' Create Trump tariff scenario matrix
#' @param countries Vector of country names
#' @param sectors Vector of sector names (if multi-sector)
#' @param baseline_year Base year for analysis
#' @return Array of tariff rates (origin x destination x sector)
create_trump_tariff_scenario <- function(countries, sectors = NULL, baseline_year = 2011) {
...
#' Compute counterfactual equilibrium with tariffs
#' @param baseline_results Baseline equilibrium results
#' @param tariff_matrix Matrix of new tariff rates
#' @param theta Trade elasticity
#' @param max_iter Maximum iterations for convergence
#' @param tolerance Convergence tolerance
#' @return Counterfactual equilibrium results
compute_counterfactual_equilibrium <- function(baseline_results, tariff_matrix, theta, 
                                              max_iter = 1000, tolerance = 1e-8) {
  
  cat("Computing counterfactual equilibrium with tariffs...\n")
  
  # Extract baseline parameters
  baseline_tau <- baseline_results$trade_costs
  T_matrix <- baseline_results$technology_params
  
  # Apply tariffs to trade costs
  new_tau <- apply_tariffs(baseline_tau, tariff_matrix)
  
  # Solve for new equilibrium
  if (is.matrix(T_matrix)) {
    # Multi-sector case
    n_countries <- nrow(T_matrix)
    n_sectors <- ncol(T_matrix)
    
    # For now, solve sector by sector (this is a simplification)
    # Full multi-sector model would solve all sectors simultaneously
    
    counterfactual_results <- list()
    
    for (s in 1:n_sectors) {
      T_vec <- T_matrix[, s]
      tau_matrix_s <- new_tau[, , s]
      
      # Assume equal labor across sectors for now (would need actual data)
      L_vec <- rep(1, n_countries)  # Placeholder
      
      equilibrium_s <- solve_equilibrium(theta, T_vec, tau_matrix_s, L_vec,
                                       max_iter = max_iter, tol = tolerance)
      
      counterfactual_results[[s]] <- equilibrium_s
    }
    
    # Aggregate results across sectors
    # This is a simplification - proper aggregation would weight by sector size
    avg_welfare <- sapply(counterfactual_results, function(x) x$welfare)
    avg_wages <- sapply(counterfactual_results, function(x) x$wages)
    
    final_results <- list(
      wages = rowMeans(avg_wages),
      welfare = rowMeans(avg_welfare),
      trade_shares = counterfactual_results[[1]]$trade_shares,  # Use first sector as proxy
      price_indices = rowMeans(sapply(counterfactual_results, function(x) x$price_indices))
    )
    
  } else {
    # Single sector case
    L_vec <- rep(1, length(T_matrix))  # Would use actual labor data
    
    final_results <- solve_equilibrium(theta, T_matrix, new_tau, L_vec,
                                     max_iter = max_iter, tol = tolerance)
  }
  
  cat("Counterfactual equilibrium computed successfully.\n")
  
  return(list(
    equilibrium = final_results,
