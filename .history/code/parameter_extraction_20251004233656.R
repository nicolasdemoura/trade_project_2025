...
get_technology_parameters <- function(processed_wiod, countries, sectors, theta = 6.53) {
    ...
    # Step 6: Handle missing values using matrix completion
    if (any(is.na(T_matrix)) || any(T_matrix <= 0)) {
        # Simple mean imputation for missing or invalid technology parameters
        for (i in 1:n_countries) {
            for (k in 1:n_sectors) {
                if (is.na(T_matrix[i, k]) || T_matrix[i, k] <= 0) {
                    sector_mean <- mean(T_matrix[, k], na.rm = TRUE)
                    country_mean <- mean(T_matrix[i, ], na.rm = TRUE)
                    T_matrix[i, k] <- ifelse(is.na(sector_mean), 
                                           ifelse(is.na(country_mean), 1, country_mean), 
                                           sector_mean)
                    if (T_matrix[i, k] <= 0) T_matrix[i, k] <- 0.01
                }
            }
        }
        cat("  - Missing/invalid values imputed:", sum(is.na(T_matrix) | T_matrix <= 0), "\n")
    }
    ...
    cat("  - Regression R-squared:", round(summary(regression_result)$r.squared, 4), "\n")
