        ...
        for (i in 1:length(countries)) {
            if (i != us_index) {
                ...
                baseline_avg_tariffs[i] <- calculate_trade_weighted_tariffs(
                    baseline_tariffs, baseline_trade_shares, baseline_sector_shares, us_index, sectors
                )
                
                # Counterfactual average tariff  
                counterfactual_avg_tariffs[i] <- calculate_trade_weighted_tariffs(
                    counterfactual_tariffs, counterfactual_trade_shares, counterfactual_sector_shares, us_index, sectors
                )
            }
        }
...
calculate_trade_weighted_tariffs <- function(tariffs, trade_shares, sector_shares, us_index, sectors) {
    country_tariffs <- 0
