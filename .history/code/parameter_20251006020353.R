...
#' Create tariff array for specific scenario from new tariff dataset
#' @param new_tariff_scenarios Processed new tariff scenarios from new_tariff_processor.R
#' @param scenario_name Name of scenario ("tariff_rate24", "tariff_rate25_YTD", etc.)
#' @param countries Vector of country names
#' @param sectors Vector of sector names
#' @param baseline_tariffs Optional baseline tariff array to merge with new data
#' @return Array of tariff rates (origin x destination x sector)
create_tariff_scenario_array <- function(new_tariff_scenarios, scenario_name, countries, sectors, baseline_tariffs = NULL) {
    
    n_countries <- length(countries)
    n_sectors <- length(sectors)
    
    # Initialize tariff array (use baseline if provided, otherwise zeros)
    if (!is.null(baseline_tariffs)) {
        tariff_array <- baseline_tariffs
        cat("✓ Using baseline tariffs as starting point\n")
    } else {
        tariff_array <- array(0, dim = c(n_countries, n_countries, n_sectors))
        cat("✓ Starting with zero tariffs\n")
    }
    
    dimnames(tariff_array) <- list(countries, countries, sectors)
    
    if (is.null(new_tariff_scenarios)) {
        cat("✓ No new tariff scenarios - using baseline only\n")
        return(tariff_array)
    }
    
    # Find USA index (these tariffs apply to US imports)
    usa_idx <- which(countries == "USA")
    if (length(usa_idx) == 0) {
        warning("USA not found in countries - cannot apply new tariff scenarios")
        return(tariff_array)
    }
    
    # Apply new tariff rates for US imports (all origins -> USA, by sector)
    updated_count <- 0
    
    for (i in 1:nrow(new_tariff_scenarios)) {
        exporter_country <- new_tariff_scenarios$Country[i]
        sector <- new_tariff_scenarios$Sector[i]
        tariff_rate <- new_tariff_scenarios[[scenario_name]][i]
        
        # Find exporter and sector indices
        exp_idx <- which(countries == exporter_country)
        sec_idx <- which(sectors == sector)
        
        if (length(exp_idx) == 1 && length(sec_idx) == 1 && !is.na(tariff_rate)) {
            # Apply tariff: exporter -> USA for this sector
            tariff_array[exp_idx, usa_idx, sec_idx] <- tariff_rate
            updated_count <- updated_count + 1
        }
    }
    
    cat("✓ New tariff scenario applied:", scenario_name, "\n")
    cat("  - Updated tariff rates:", updated_count, "\n")
    cat("  - USA index:", usa_idx, "\n")
    cat("  - Non-zero tariffs total:", sum(tariff_array > 0), "\n")
    if (sum(tariff_array > 0) > 0) {
        cat("  - Average non-zero tariff:", round(mean(tariff_array[tariff_array > 0]) * 100, 2), "%\n")
    }
...
#' @param use_new_tariffs Logical, whether to use new tariff scenarios
#' @return List containing processed data and all model parameters
process_and_extract_all_parameters <- function(raw_database, theta = 6.53, use_new_tariffs = FALSE) {
    ...
    baseline_tariffs <- get_tariff_data(processed_tariffs, countries, sectors)
    
    # Create tariff scenarios if new data available
    tariff_scenarios <- list(baseline = baseline_tariffs)
    
    if (use_new_tariffs && !is.null(raw_database$new_tariff_scenarios)) {
        cat("Creating multiple tariff scenarios from new data...\n")
        
        scenario_names <- c("tariff_rate24", "tariff_rate25_YTD", "tariff_rate25_LTM", "tariff_rate25_3M")
        
        for (scenario in scenario_names) {
            tariff_scenarios[[scenario]] <- create_tariff_scenario_array(
                raw_database$new_tariff_scenarios, 
                scenario, 
                countries, 
                sectors, 
                baseline_tariffs
    ...
    parameters <- list(
        ...
        tariffs = baseline_tariffs,  # Keep baseline as default
        tariff_scenarios = tariff_scenarios  # All scenarios including baseline
    )
