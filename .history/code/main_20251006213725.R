...
modelsummary::modelsummary(as.matrix(model_parameters$alpha), output = "tables/alpha_table.tex", 
                           title = "Expenditure Shares (Alpha)", 
                           note = "This table presents the expenditure shares (alpha) for each country-sector pair.", 
                           escape = FALSE)

modelsummary::modelsummary(model_parameters$beta, output = "tables/beta_table.tex", 
                           title = "Labor Shares (Beta)", 
                           note = "This table presents the labor shares (beta) for each country-sector pair.", 
                           escape = FALSE)
...
for(j in 1:dim(model_parameters$gamma)[1]) {
    ...
    modelsummary::modelsummary(gamma_matrix, output = paste0("tables/gamma_table_", country, ".tex"), 
                               title = paste("Intermediate Input Shares (Gamma) -", country), 
                               note = paste("This table presents the intermediate input shares (gamma) for", country, "across all sectors."), 
                               escape = FALSE)
}
