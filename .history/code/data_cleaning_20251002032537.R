...
###############################################################################
# Clean WIOD 201    final_data <- list(
        # WIOD data (country-aggregated, filtered, and trade-balanced)
        trade_flows_array = trade_flows_array,
        observed_trade_shares = observed_trade_shares,
        absorption_matrix = absorption_matrix,
        
        # Country and sector info (aggregated)
        countries = countries,  # Aggregated countries with complete data
        sectors = sectors,
        
        # Socio-economic data (country-aggregated)  
        ...
        # Additional metadata
        country_groups_applied = country_groups,
        trade_balanced = TRUE,
        aggregation_method = \"flexible_country_grouping\"
    )) methodology
###############################################################################
