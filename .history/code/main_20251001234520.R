...
# Load Socio_Economic_Accounts_July14.xlsx (for exploration only - actual analysis uses file path)
socioeco <- read_excel("code/data/Socio_Economic_Accounts_July14.xlsx", sheet = 2)
...
# Examine the actual WIOD structure
if ("row_country" %in% colnames(wiot_full)) {
  cat("\nUnique countries in WIOD (from row_country):\n")
  countries_sample <- sort(unique(wiot_full$row_country))
  print(countries_sample)
  cat("Total countries:", length(countries_sample), "\n")
}

if ("row_item" %in% colnames(wiot_full)) {
  cat("\nSample sectors/items in WIOD (from row_item):\n")
  items_sample <- unique(wiot_full$row_item)
  print(head(items_sample, 20))  # Show first 20
  cat("Total items:", length(items_sample), "\n")
}
...
if ("value" %in% colnames(wiot_full)) {
  cat("\nValue statistics:\n")
  cat("Range:", range(wiot_full$value, na.rm = TRUE), "\n")
  cat("Mean:", round(mean(wiot_full$value, na.rm = TRUE), 2), "\n")
  cat("Missing values:", sum(is.na(wiot_full$value)), "\n")
}
...
cleaned_data <- clean_all_data_cdk(wiot_full, socioeco_file_path, year = 2011)
...
if (!is.null(cleaned_data$trade_flows_array)) {
  ...
  cat("\nCDK (2012) results saved to Tables/ directory\n")
  cat("CDK analysis completed successfully!\n")
  
} else {
  cat("CDK data cleaning incomplete - check data structure and update cleaning functions.\n")
}
