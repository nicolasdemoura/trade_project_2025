        ...
        for (i in 1:length(countries)) {
            if (i != us_index) {
                ...
                baseline_avg_tariffs[i] <- calculate_trade_weighted_tariffs(
                    baseline_tariffs, baseline_trade_shares, baseline_sector_shares, us_index, countries, sectors
                )
                
                # Counterfactual average tariff  
                counterfactual_avg_tariffs[i] <- calculate_trade_weighted_tariffs(
                    counterfactual_tariffs, counterfactual_trade_shares, counterfactual_sector_shares, us_index, countries, sectors
                )
            }
        }
...
calculate_trade_weighted_tariffs <- function(tariffs, trade_shares, sector_shares, us_index, countries, sectors) {
    country_tariffs <- numeric(length(countries))

    total_imports <- 0
    weighted_tariff_sum <- 0
    
    for (k in 1:length(sectors)) {
        import_value <- trade_shares[i, us_index, k] * sector_shares[k]
    ...
    if (total_imports > 0) {
        country_tariffs[i] <- weighted_tariff_sum / total_imports
    }
