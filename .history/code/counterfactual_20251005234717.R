...
solve_counterfactual_equilibrium <- function(database, parameters, fixed_iceberg_costs, labor_mobility = TRUE) {
    ...
    counterfactual_objective <- function(params) {
        ...
        # Add penalties for extreme parameter values (in log scale)
        penalty <- 0
        if (any(params < -5, na.rm = TRUE) | any(params > 5, na.rm = TRUE)) {
            penalty <- penalty + 1000  # Penalize extreme log values (exp(-5) ≈ 0.007, exp(5) ≈ 148)
        }
    ...
    optimization_result <- optim(
        ...
        control = list(
            ...
            abstol = 1e-6,   # Tighter convergence with price consistency
            trace = 1,
            REPORT = 10
        )
    )
