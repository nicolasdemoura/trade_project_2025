...
#' @param expenditure_matrix To    cat("Calibrating multi-sector baseline equilibrium...\n")
    ...
    if (!is.null(cleaned_data$observed_trade_shares)) {
        cat("Using trade share matching calibration...\n")
        ...
        cat("Using iterative equilibrium solver...\n")
    ...
    }nditure by country (countries x sectors)
...
calibrate_multisector_baseline <- function(T_matrix, cleaned_data, tau_array = NULL, theta = NULL, labor_mobility = TRUE) {
...
#' Calibrate equilibrium by directly matching observed trade shares
...
#' @return Equilibrium object with calibrated wages, prices, and matched trade shares
calibrate_equilibrium_from_trade_shares <- function(theta, T_matrix, tau_array, labor_matrix, beta_matrix, gamma_matrix, expenditure_matrix, observed_trade_shares, labor_mobility = FALSE) {
    
    n_countries <- nrow(T_matrix)
    n_sectors <- ncol(T_matrix)
    
    cat("Calibrating equilibrium by matching observed trade shares...\n")
    cat("Labor mobility mode:", ifelse(labor_mobility, "Mobile", "Immobile"), "\n")
    
    # Step 1: Use observed trade shares directly
    trade_shares <- observed_trade_shares
    
    # Step 2: Initialize wages and prices
    wages <- matrix(1, n_countries, n_sectors)
    wages[1, ] <- 1  # Normalize first country wages
    price_indices <- array(1, dim = c(n_countries, n_sectors))
    
    # Step 3: Calibrate wages from labor market clearing and trade shares
    # For each non-numeraire country, solve for wages that satisfy:
    # w_ik * L_ik = β_ik * Σ_n π_nik * X_nk (immobile labor)
    # or w_i * Σ_k L_ik = Σ_k β_ik * Σ_n π_nik * X_nk (mobile labor)
    
    if (labor_mobility) {
        # Mobile labor: solve for common wage per country
        for (i in 1:n_countries) {
            if (i > 1) {  # Skip numeraire country
                ...
                if (total_labor > 0) {
                    common_wage <- total_revenue / total_labor
                    wages[i, ] <- common_wage
                }
            }
        }
    } else {
        # Immobile labor: solve for sector-specific wages
        for (i in 1:n_countries) {
            if (i > 1) {  # Skip numeraire country
                for (k in 1:n_sectors) {
                    if (labor_matrix[i, k] > 0) {
                        ...
                        for (n in 1:n_countries) {
                            sector_revenue <- sector_revenue + 
                                trade_shares[i, n, k] * expenditure_matrix[n, k]
                        }
                        wages[i, k] <- (beta_matrix[i, k] * sector_revenue) / labor_matrix[i, k]
                    }
                }
            }
        }
    ...
    # Step 4: Calculate unit costs and price indices consistent with calibrated wages
    unit_costs <- array(0, dim = c(n_countries, n_sectors))
    
    # Calculate unit costs: c_ik = (w_ik)^β_ik * (∏_{k'} p_ik'^γ_{ik'})^{1-β_ik}
    for (i in 1:n_countries) {
        for (k in 1:n_sectors) {
            # Start with labor component
            labor_cost <- wages[i, k]^beta_matrix[i, k]
            
            # For simplicity, assume intermediate inputs use domestic prices initially
            # This can be refined in subsequent iterations if needed
            intermediate_cost <- 1
            for (kprime in 1:n_sectors) {
                # Use a reasonable approximation for price indices
                # Can be improved with more sophisticated calibration
                price_approx <- mean(wages[i, ])  # Simple approximation
                intermediate_cost <- intermediate_cost * (price_approx^gamma_matrix[i, k, kprime])
            }
    ...
    # Calculate final price indices from trade share consistency
    for (n in 1:n_countries) {
        for (k in 1:n_sectors) {
            # p_nk^{-θ} = Σ_i T_ik (c_ik * d_nik)^{-θ}
            sum_competitors <- 0
            for (i in 1:n_countries) {
    ...
    for (i in 1:n_countries) {
        ...
        if (total_labor > 0) {
            ...
            # Price level (Cobb-Douglas aggregation)
            price_level <- 1
            for (k in 1:n_sectors) {
    ...
    cat("Trade share matching calibration completed.\n")
    cat("Average domestic trade share:", round(mean(sapply(1:n_sectors, function(k) mean(diag(trade_shares[,,k])))), 3), "\n")
    
    return(list(
        wages = wages,
        price_indices = price_indices,
        trade_shares = trade_shares,  # Exactly matched to observed data
        unit_costs = unit_costs,
        gdp = gdp,
        welfare = welfare,
        method = "trade_share_matching"
    ))
}

#' Solve multi-sector input-output equilibrium with endogenous wages and prices (iterative method)
#' @param theta Trade elasticity parameter
#' @param T_matrix Technology matrix (countries x sectors)
#' @param tau_array Trade cost array (origin x destination x sectors, domestic costs = 1)
#' @param labor_matrix Labor allocation matrix (countries x sectors)
...
solve_equilibrium_cdk_multi_sector <- function(theta, T_matrix, tau_array, labor_matrix, beta_matrix, gamma_matrix, expenditure_matrix, labor_mobility = TRUE, max_iter = 1000, tol = 1e-8) {
    ...
    for (iter in 1:max_iter) {
        ...
        # View NA or Inf values in trade_shares
        if (any(is.na(trade_shares)) || any(is.infinite(trade_shares))) {
          cat("Warning: NA or Inf values detected in trade shares at iteration", iter, "\n")
          cat(" They are at positions:\n")
          print(which(is.na(trade_shares) | is.infinite(trade_shares), arr.ind = TRUE))
        }
        ...
        if (labor_mobility) {
            ...
            for (i in 1:n_countries) {
                if (i > 1) {  # Skip first country (numeraire)
                    ...
                    total_labor <- sum(labor_matrix[i, ], na.rm = TRUE)
        ...
        # Check convergence
        wage_change <- max(abs(wages - wages_old) / pmax(abs(wages_old), 1e-6))
        price_change <- max(abs(price_indices - price_indices_old) / pmax(abs(price_indices_old), 1e-6))
