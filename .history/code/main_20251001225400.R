...
cleaned_data <- clean_all_data(wiot_full, socioeco, year = 2011)

# Step 2: Estimate trade elasticity (use CDK 2012 benchmark)
cat("Step 2: Setting trade elasticity parameter...\n")
theta <- estimate_trade_elasticity(NULL)  # Uses CDK benchmark

# Step 3: Prepare data for regression analysis
if (!is.null(cleaned_data$trade_matrices)) {
  cat("Step 3: Preparing regression data...\n")
  
  countries <- cleaned_data$wiod_clean$countries
  sectors <- cleaned_data$wiod_clean$sectors
  
  if (is.null(countries) || is.null(sectors)) {
    cat("Warning: Country or sector information not extracted properly.\n")
    cat("Will need to implement data cleaning functions based on actual data structure.\n")
  } else {
