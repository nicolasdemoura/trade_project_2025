...
clean_wiod_data_cdk <- function(wiot_file_path, target_year = 2009, country_groups = NULL, sector_groups = NULL) {
...
load_tariff_data <- function(tariff_file_path = NULL, country_groups = NULL, sector_groups = NULL) {
    
    n_countries <- length(country_groups)
    n_sectors <- length(sector_groups)
    
    tariff_data <- read.csv(tariff_file_path, stringsAsFactors = FALSE)
    tariff_data <- as.data.frame(tariff_data)
    tariff_data <- tariff_data %>% 
      dplyr::select(`Reporter Name`, `Product Name`, `Partner Name`, `Weighted Average`) %>%
      rename(importer = `Reporter Name`, sector = `Product Name`, exporter = `Partner Name`, tariff_rate = `Weighted Average`) %>%
      mutate(importer = ifelse(importer == "EuropeanUnion", "EU", importer),
             importer = ifelse(importer == "RestOfTheWorld", "RoW", importer),
             exporter = ifelse(exporter == "EuropeanUnion", "EU", exporter),
             exporter = ifelse(exporter == "RestOfTheWorld", "RoW", exporter),
             sector = paste0("c", sector))
    
    # Create tariff array
    tariff_array <- array(0, dim = c(n_countries, n_countries, n_sectors),
                         dimnames = list(country_groups, country_groups, sectors))
...
#' @param target_year Target year for analysis (default: 2009)
#' @param country_groups Named list of country groups for aggregation (applied before cleaning)
#' @param sector_groups Named list of sector groups for aggregation (applied before cleaning)
#' @return List with cleaned labor data and countries with complete data
clean_socioeconomic_data_cdk <- function(socioeco_file_path, exchange_rates_file_path, target_year = 2009, country_groups = NULL, sector_groups = NULL) {
    ...
    # Filter countries with complete 2009 data
...
clean_all_data_cdk <- function(wiot_file_path, socioeco_file_path, exchange_rates_file_path, tariff_file_path, target_year = 2009, country_groups = NULL, sector_groups = NULL, ) {
