...
#' @param r    # Step 2: Extract parameters from processed data
    ...
    parameters <- list(
        ...
        technology = get_technology_parameters(processed_wiod, raw_database, countries, sectors, theta),
        theta = get_trade_elasticity(theta),
        tariffs = get_tariff_data(processed_tariffs, countries, sectors)
    )e Raw c    # Step 4: Extracting model parameters...
    parameters <- list(
        alpha = get_alpha_parameters(processed_wiod, countries, sectors),
        beta = get_beta_parameters(processed_socioeco, countries, sectors),
        gamma = get_gamma_parameters(processed_wiod, countries, sectors),
...
#' Extract and calibrate technology parameters using regression approach from original estimation.R
#' @param processed_wiod Processed WIOD data from process_wiod_data()
#' @param raw_database Raw database containing socioeconomic data for productivity calculation
#' @param countries Vector of country names
#' @param sectors Vector of sector names
#' @param theta Trade elasticity parameter
#' @return Matrix of technology parameters (countries x sectors)
get_technology_parameters <- function(processed_wiod, raw_database, countries, sectors, theta = 6.53) {
    
    n_countries <- length(countries)
    n_sectors <- length(sectors)
    
    # Use regression approach from original estimation.R
    cat("✓ Technology parameters estimated using regression approach\n")
    
    # Prepare data for regression estimation
    # This uses the original methodology from your estimation.R file
    
    # Create productivity database for regression
    productivity_database <- list(
        # WIOD processed data
        trade_flows_array = processed_wiod$trade_flows_array,
        observed_trade_shares = processed_wiod$observed_trade_shares,
        
        # Raw socioeconomic data for productivity calculation
        socioeco_data = raw_database$socioeco_data,
    ...
    # Call the original technology parameter estimation function
    # This function should exist in your estimation.R file
    technology_results <- estimate_technology_parameters(
        database = productivity_database,
        theta = theta
    )
    
    # Extract technology matrix from estimation results
    if ("technology_matrix" %in% names(technology_results)) {
        T_matrix <- technology_results$technology_matrix
    } else if ("T_matrix" %in% names(technology_results)) {
        T_matrix <- technology_results$T_matrix
    } else {
        # Fallback to the main results object if it's directly the matrix
        T_matrix <- technology_results
    }
    
    # Ensure proper matrix structure
    if (!is.matrix(T_matrix) || nrow(T_matrix) != n_countries || ncol(T_matrix) != n_sectors) {
        stop("Technology parameter estimation failed - invalid matrix dimensions")
    }
    ...
    if (any(is.na(T_matrix)) || any(T_matrix <= 0)) {
        warning("Technology parameter estimation produced invalid values")
        # Replace invalid values with small positive number
        T_matrix[is.na(T_matrix) | T_matrix <= 0] <- 0.1
    }
