...
#' Load and clean tariff data for CDK analysis
...
load_tariff_data <- function(tariff_file_path = NULL, target_year = 2011, country_groups = NULL, sector_groups = NULL, countries, sectors) {
    
    n_countries <- length(countries)
    n_sectors <- length(sectors)
    
    # If no tariff file provided, return zero tariffs
    if (is.null(tariff_file_path) || !file.exists(tariff_file_path)) {
        cat("No tariff data file provided - assuming zero baseline tariffs\\n")
        return(array(0, dim = c(n_countries, n_countries, n_sectors),
                    dimnames = list(countries, countries, sectors)))
    }
    
    # Load tariff data based on file extension
    if (grepl("\\.csv$", tariff_file_path, ignore.case = TRUE)) {
        tariff_data <- read.csv(tariff_file_path, stringsAsFactors = FALSE)
    } else if (grepl("\\.(xlsx|xls)$", tariff_file_path, ignore.case = TRUE)) {
        tariff_data <- readxl::read_excel(tariff_file_path)
        tariff_data <- as.data.frame(tariff_data)
    } else {
        cat("Unsupported tariff file format - assuming zero tariffs\\n")
        return(array(0, dim = c(n_countries, n_countries, n_sectors),
                    dimnames = list(countries, countries, sectors)))
    }
    
    # Expect columns: importer, exporter, sector, tariff_rate (and possibly year)
    required_cols <- c("importer", "exporter", "sector", "tariff_rate")
    if (!all(required_cols %in% names(tariff_data))) {
        cat("Tariff data missing required columns - assuming zero tariffs\\n")
        return(array(0, dim = c(n_countries, n_countries, n_sectors),
                    dimnames = list(countries, countries, sectors)))
    }
    
    # Filter by year if year column exists
    if ("year" %in% names(tariff_data)) {
        tariff_data <- tariff_data[tariff_data$year == target_year, ]
    }
    
    # Apply country and sector aggregation to tariff data
    if (!is.null(country_groups)) {
        ...
        # Map tariff data countries
        tariff_data$importer <- sapply(tariff_data$importer, function(x) {
            ifelse(x %in% names(country_mapping), country_mapping[[x]], x)
        })
        tariff_data$exporter <- sapply(tariff_data$exporter, function(x) {
            ifelse(x %in% names(country_mapping), country_mapping[[x]], x)
        })
        
        # Aggregate by taking averages (could also use trade-weighted averages)
        tariff_data <- tariff_data %>%
            group_by(importer, exporter, sector) %>%
            summarise(tariff_rate = mean(tariff_rate, na.rm = TRUE), .groups = "drop")
    }
    
    if (!is.null(sector_groups)) {
        # Create mapping for sectors
        sector_mapping <- list()
        for (group_name in names(sector_groups)) {
            for (sector in sector_groups[[group_name]]) {
                sector_mapping[[sector]] <- group_name
        ...
        # Map tariff data sectors
        tariff_data$sector <- sapply(tariff_data$sector, function(x) {
            ifelse(x %in% names(sector_mapping), sector_mapping[[x]], x)
        })
        
        # Aggregate by taking averages
        tariff_data <- tariff_data %>%
            group_by(importer, exporter, sector) %>%
            summarise(tariff_rate = mean(tariff_rate, na.rm = TRUE), .groups = "drop")
    }
    ...
    # Fill tariff array
    for (i in 1:nrow(tariff_data)) {
        imp_idx <- which(countries == tariff_data$importer[i])
        exp_idx <- which(countries == tariff_data$exporter[i])
        sec_idx <- which(sectors == tariff_data$sector[i])
        
        if (length(imp_idx) > 0 && length(exp_idx) > 0 && length(sec_idx) > 0) {
            # Note: tariff_array[exporter, importer, sector] because trade costs are from exporter to importer
            tariff_array[exp_idx, imp_idx, sec_idx] <- tariff_data$tariff_rate[i]
        }
    }
    
    cat("Loaded tariff data for", sum(tariff_array > 0), "country-sector pairs\\n")
    cat("Average tariff rate:", round(mean(tariff_array[tariff_array > 0]), 4), "\\n")
...
#' @param country_groups Named list of country groups for aggregation (applied before cleaning)
#' @param sector_groups Named list of sector groups for aggregation (applied before cleaning)
#' @param tariff_file_path Optional path to tariff data file (CSV or Excel)
#' @return List with all cleaned and processed data for CDK methodology
clean_all_data_cdk <- function(wiot_file_path, socioeco_file_path, exchange_rates_file_path, target_year = 2011, country_groups = NULL, sector_groups = NULL, tariff_file_path = NULL) {
  
    # Load and clean WIOD data
    wiod_cleaned <- clean_wiod_data_cdk(wiot_file_path, target_year, country_groups, sector_groups)
    socio_cleaned <- clean_socioeconomic_data_cdk(socioeco_file_path, exchange_rates_file_path, target_year, country_groups, sector_groups)

    # Get the countries and sectors from cleaned data
    ...
    # Load tariff data 
    tariffs <- load_tariff_data(tariff_file_path, target_year, country_groups, sector_groups, countries, sectors)
    
    # Extract cleaned components
